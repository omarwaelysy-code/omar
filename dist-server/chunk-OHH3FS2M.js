...
import { relative as relative2, dirname, basename, extname, resolve as resolve$1, join } from "path";
import { posix, isAbsolute as isAbsolute2, resolve, win32 } from "path";
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, importerAttributes, fs2) {
  ...
  if (importer !== void 0 && !isAbsolute(source) && source[0] !== ".")
    return null;
  return addJsExtensionIfNecessary(importer ? resolve$1(dirname(importer), source) : resolve$1(source), preserveSymlinks, fs2);
}
...
async function findFile(file, preserveSymlinks, fs2) {
  try {
    ...
    if (preserveSymlinks && stats.isSymbolicLink() || stats.isFile()) {
      ...
      const files = await fs2.readdir(dirname(file));
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-IVHGWZ74.js");
