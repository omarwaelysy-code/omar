-- COMPREHENSIVE SCHEMA AUDIT MIGRATION
-- This migration ensures that all tables have the latest required columns 
-- that might have been added to init-db.ts but missed in existing databases.
...
ALTER TABLE invoices ADD COLUMN IF NOT EXISTS payment_type VARCHAR(20) DEFAULT 'cash';
