...
-- =============================================================================
-- RECONCILIATION BLOCK (runs every startup, before numbered migrations)
-- Phases A-G from migration 023 — embedded here so old production databases
-- are fully repaired BEFORE migrations 017-022 attempt to execute.
-- All statements are idempotent (IF NOT EXISTS / existence-guarded).
-- =============================================================================
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'currencies') THEN
    CREATE TABLE currencies (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36),
        code VARCHAR(10) NOT NULL, name_ar VARCHAR(100) NOT NULL,
        name_en VARCHAR(100) NOT NULL, symbol VARCHAR(10),
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'exchange_rates') THEN
    CREATE TABLE exchange_rates (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), currency_id VARCHAR(36),
        exchange_rate DECIMAL(18,6) NOT NULL, rate_date DATE NOT NULL,
        notes TEXT, created_by VARCHAR(36), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
END IF;

IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'departments') THEN
    CREATE TABLE departments (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), code VARCHAR(50) UNIQUE,
        name VARCHAR(255) NOT NULL, description TEXT, parent_id VARCHAR(36),
        manager_user_id VARCHAR(36), is_active BOOLEAN DEFAULT true,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cost_centers') THEN
    CREATE TABLE cost_centers (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), department_id VARCHAR(36),
        code VARCHAR(50) UNIQUE, name VARCHAR(255) NOT NULL, description TEXT,
        budget DECIMAL(18,4), currency VARCHAR(10), is_active BOOLEAN DEFAULT true,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'operation_categories') THEN
    CREATE TABLE operation_categories (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), name VARCHAR(255) NOT NULL,
        code VARCHAR(50), parent_id VARCHAR(36), is_final BOOLEAN DEFAULT FALSE,
        level INT DEFAULT 0, full_path TEXT, description TEXT,
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'operation_fields') THEN
    CREATE TABLE operation_fields (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), category_id VARCHAR(36),
        operation_category_id VARCHAR(36), name VARCHAR(255) NOT NULL, label VARCHAR(255),
        code VARCHAR(50) UNIQUE, description TEXT, type VARCHAR(50) NOT NULL,
        unit VARCHAR(50), default_value TEXT, is_required BOOLEAN DEFAULT false,
        options JSONB, sort_order INTEGER DEFAULT 0, department_id VARCHAR(36),
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'operations') THEN
    CREATE TABLE operations (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), category_id VARCHAR(36),
        operation_category_id VARCHAR(36), department_id VARCHAR(36), cost_center_id VARCHAR(36),
        operation_number VARCHAR(50) UNIQUE, operation_date DATE, date DATE,
        customer_id VARCHAR(36), customer_name VARCHAR(255), description TEXT,
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'operation_field_values') THEN
    CREATE TABLE operation_field_values (
        id VARCHAR(36) PRIMARY KEY, operation_id VARCHAR(36), field_id VARCHAR(36),
        value TEXT, company_id VARCHAR(36), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'field_operation_categories') THEN
    CREATE TABLE field_operation_categories (
        ...
        field_id VARCHAR(36), category_id VARCHAR(36), company_id VARCHAR(36),
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'purchase_return_items') THEN
    CREATE TABLE purchase_return_items (
        id VARCHAR(36) PRIMARY KEY, return_id VARCHAR(36), product_id VARCHAR(36),
        company_id VARCHAR(36), description TEXT, quantity DECIMAL(18,4) NOT NULL,
        unit_price DECIMAL(18,4) NOT NULL, total DECIMAL(18,4) NOT NULL,
        product_name VARCHAR(255), product_code VARCHAR(100), product_image_url TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_logs') THEN
    CREATE TABLE audit_logs (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), user_id VARCHAR(36),
        username VARCHAR(255), user_email VARCHAR(255), action VARCHAR(100) NOT NULL,
        module VARCHAR(100), details TEXT, entity_type VARCHAR(100), entity_id VARCHAR(100),
...
IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'item_groups') THEN
    CREATE TABLE item_groups (
        id VARCHAR(36) PRIMARY KEY, company_id VARCHAR(36), name VARCHAR(255) NOT NULL,
        code VARCHAR(100) NOT NULL, type VARCHAR(100) NOT NULL, sequence_number INTEGER DEFAULT 1,
...
-- currencies / exchange_rates extra columns
...
-- Drop all FK constraints from affected tables first, then convert.
-- ============================================================
...
-- Convert each UUID column only if still uuid type
IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_categories') THEN
...
-- ============================================================
-- PHASE D: ORPHAN DATA CLEANUP (before FK re-creation)
-- ============================================================

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operations') AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_categories') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operations' AND column_name='category_id') THEN
        UPDATE operations SET category_id = NULL WHERE category_id IS NOT NULL AND category_id::text NOT IN (SELECT id::text FROM operation_categories);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operations' AND column_name='operation_category_id') THEN
        UPDATE operations SET operation_category_id = NULL WHERE operation_category_id IS NOT NULL AND operation_category_id::text NOT IN (SELECT id::text FROM operation_categories);
    END IF;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operations') AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='departments') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operations' AND column_name='department_id') THEN
        UPDATE operations SET department_id = NULL WHERE department_id IS NOT NULL AND department_id::text NOT IN (SELECT id::text FROM departments);
    END IF;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operations') AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='cost_centers') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operations' AND column_name='cost_center_id') THEN
        UPDATE operations SET cost_center_id = NULL WHERE cost_center_id IS NOT NULL AND cost_center_id::text NOT IN (SELECT id::text FROM cost_centers);
    END IF;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_fields') AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_categories') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operation_fields' AND column_name='category_id') THEN
        UPDATE operation_fields SET category_id = NULL WHERE category_id IS NOT NULL AND category_id::text NOT IN (SELECT id::text FROM operation_categories);
    END IF;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='cost_centers') AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='departments') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='cost_centers' AND column_name='department_id') THEN
        UPDATE cost_centers SET department_id = NULL WHERE department_id IS NOT NULL AND department_id::text NOT IN (SELECT id::text FROM departments);
    END IF;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='field_operation_categories') THEN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_categories') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='field_operation_categories' AND column_name='category_id') THEN
        DELETE FROM field_operation_categories WHERE category_id IS NOT NULL AND category_id::text NOT IN (SELECT id::text FROM operation_categories);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_fields') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='field_operation_categories' AND column_name='field_id') THEN
        DELETE FROM field_operation_categories WHERE field_id IS NOT NULL AND field_id::text NOT IN (SELECT id::text FROM operation_fields);
    END IF;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_field_values') THEN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operations') THEN
        DELETE FROM operation_field_values WHERE operation_id IS NOT NULL AND operation_id::text NOT IN (SELECT id::text FROM operations);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_fields') THEN
        DELETE FROM operation_field_values WHERE field_id IS NOT NULL AND field_id::text NOT IN (SELECT id::text FROM operation_fields);
    END IF;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='operation_categories') THEN
    UPDATE operation_categories SET parent_id = NULL WHERE parent_id IS NOT NULL AND parent_id::text NOT IN (SELECT id::text FROM operation_categories);
END IF;
IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='departments') THEN
    UPDATE departments SET parent_id = NULL WHERE parent_id IS NOT NULL AND parent_id::text NOT IN (SELECT id::text FROM departments);
END IF;
...
IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='operation_categories_parent_id_fkey' AND table_name='operation_categories') THEN
    ALTER TABLE operation_categories ADD CONSTRAINT operation_categories_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES operation_categories(id) ON DELETE SET NULL;
END IF;

IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='departments_parent_id_fkey' AND table_name='departments') THEN
...
AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='cost_centers_department_id_fkey' AND table_name='cost_centers') THEN
    ALTER TABLE cost_centers ADD CONSTRAINT cost_centers_department_id_fkey FOREIGN KEY (department_id) REFERENCES departments(id) ON DELETE SET NULL;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operation_fields' AND column_name='category_id')
AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='operation_fields_category_id_fkey' AND table_name='operation_fields') THEN
    ALTER TABLE operation_fields ADD CONSTRAINT operation_fields_category_id_fkey FOREIGN KEY (category_id) REFERENCES operation_categories(id) ON DELETE SET NULL;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operations' AND column_name='category_id')
AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='operations_category_id_fkey' AND table_name='operations') THEN
    ALTER TABLE operations ADD CONSTRAINT operations_category_id_fkey FOREIGN KEY (category_id) REFERENCES operation_categories(id) ON DELETE SET NULL;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operations' AND column_name='department_id')
AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='operations_department_id_fkey' AND table_name='operations') THEN
    ALTER TABLE operations ADD CONSTRAINT operations_department_id_fkey FOREIGN KEY (department_id) REFERENCES departments(id) ON DELETE SET NULL;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='operations' AND column_name='cost_center_id')
AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='operations_cost_center_id_fkey' AND table_name='operations') THEN
    ALTER TABLE operations ADD CONSTRAINT operations_cost_center_id_fkey FOREIGN KEY (cost_center_id) REFERENCES cost_centers(id) ON DELETE SET NULL;
END IF;

IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='operation_field_values_operation_id_fkey' AND table_name='operation_field_values') THEN
    ALTER TABLE operation_field_values ADD CONSTRAINT operation_field_values_operation_id_fkey FOREIGN KEY (operation_id) REFERENCES operations(id) ON DELETE CASCADE;
END IF;

IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='operation_field_values_field_id_fkey' AND table_name='operation_field_values') THEN
    ALTER TABLE operation_field_values ADD CONSTRAINT operation_field_values_field_id_fkey FOREIGN KEY (field_id) REFERENCES operation_fields(id) ON DELETE CASCADE;
END IF;

IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='field_operation_categories' AND column_name='field_id')
AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='field_operation_categories_field_id_fkey' AND table_name='field_operation_categories') THEN
    ALTER TABLE field_operation_categories ADD CONSTRAINT field_operation_categories_field_id_fkey FOREIGN KEY (field_id) REFERENCES operation_fields(id) ON DELETE CASCADE;
END IF;
...
AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='field_operation_categories_category_id_fkey' AND table_name='field_operation_categories') THEN
    ALTER TABLE field_operation_categories ADD CONSTRAINT field_operation_categories_category_id_fkey FOREIGN KEY (category_id) REFERENCES operation_categories(id) ON DELETE CASCADE;
END IF;
...
-- Final schema version stamp
INSERT INTO _system_settings (key, value)
VALUES ('schema_version', '{"version": "2.1.0", "reconciled_at": "2026-06-04", "embedded_in_master": true}')
