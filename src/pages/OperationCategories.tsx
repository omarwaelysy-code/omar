...
export function OperationCategories() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500" dir="rtl">
      ...
      <div className="bg-slate-50/30 p-6 rounded-[2.5rem] border border-slate-100">
        {loading ? (
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" dir="rtl">
            <motion.div
              initial={{ opacity: 0, y: 10, scale: 0.98 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: 10, scale: 0.98 }}
              className="bg-white rounded-[2rem] w-full max-w-lg overflow-hidden shadow-2xl relative border border-slate-200"
            >
              <button 
                onClick={() => setIsModalOpen(false)}
                className="absolute top-6 left-6 p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-50 rounded-xl transition-all"
              >
                <X size={20} />
              </button>
              ...
              <div className="p-10 border-b border-slate-50">
                ...
                <p className="text-slate-500 text-sm font-medium">أدخل البيانات الأساسية للتصنيف داخل الشجرة</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-10 space-y-6">
                ...
                <div className="flex items-center gap-3 pt-6 border-t border-slate-50">
                  <button
                    type="submit"
                    className="flex-1 bg-emerald-600 text-white h-14 rounded-2xl font-bold text-lg hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-500/20 active:scale-95"
                  >
                    حفظ التعديلات
                  </button>
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="flex-1 bg-slate-50 text-slate-600 h-14 rounded-2xl font-bold text-lg hover:bg-slate-100 transition-all"
                  >
                    إلغاء
                  </button>
                </div>
