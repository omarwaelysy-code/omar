...
export const Returns: React.FC = () => {
  ...
  const proceedEtaUpload = async (returnDoc: Return) => {
    ...
    if (taxNumberError || itemsError) {
      setEtaValidationModal({
        ...
        customerName: cust?.name || returnDoc.customer_name || 'العميل',
        taxNumberError,
        itemsError,
        unregisteredItems
      });
    ...
    try {
      ...
      if (res && (res.success || res.uuid)) {
        ...
        if (res?.taxNumberError || res?.itemsError) {
          setEtaValidationModal({
            isOpen: true,
            title: language === 'ar' ? 'تنبيه متطلبات الفاتورة الإلكترونية' : 'ETA Requirements Alert',
            customerName: res.customerName || cust?.name || returnDoc.customer_name,
      ...
      if (msg.includes('رقم ضريبي') || msg.includes('الاصناف غير مسجلة')) {
        setEtaValidationModal({
          isOpen: true,
          title: language === 'ar' ? 'تنبيه متطلبات الفاتورة الإلكترونية' : 'ETA Requirements Alert',
          customerName: cust?.name || returnDoc.customer_name,
  ...
  const addItem = (productId: string) => {
    ...
    const whtRate = Number(product.sales_withholding_tax_rate) || 0;
    const whtAmount = Number((foreignPrice * (whtRate / 100)).toFixed(2));
    setItems(prev => [...prev, {
    ...
    setItems(prev => {
      ...
      if (field === 'product_id') {
        ...
        if (product) {
          ...
          item.withholding_tax_rate = Number(product.sales_withholding_tax_rate) || 0;
          const total = (Number(item.quantity) || 0) * foreignPrice;
          item.total = total;
          item.vat_amount = isVatEnabled ? Number((total * ((item.vat_rate || 0) / 100)).toFixed(2)) : 0;
          item.withholding_tax_amount = Number((total * ((item.withholding_tax_rate || 0) / 100)).toFixed(2));
        } else {
          item.product_name = '';
          item.product_code = '';
      ...
      if (field === 'quantity' || field === 'unit_price' || field === 'vat_rate' || field === 'withholding_tax_rate') {
        ...
        if ((item.withholding_tax_rate === undefined || item.withholding_tax_rate === null || item.withholding_tax_rate === 0) && item.product_id && field !== 'withholding_tax_rate') {
          const p = products.find(prod => prod.id === item.product_id);
          if (p && p.sales_withholding_tax_rate) item.withholding_tax_rate = Number(p.sales_withholding_tax_rate);
        }
        const total = (Number(item.quantity) || 0) * (Number(item.unit_price) || 0);
        item.total = total;
        item.vat_amount = isVatEnabled ? Number((total * ((item.vat_rate || 0) / 100)).toFixed(2)) : 0;
        item.withholding_tax_amount = Number((total * ((Number(item.withholding_tax_rate) || 0) / 100)).toFixed(2));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const whtTotal = validItems.reduce((sum, item) => sum + (Number(item.withholding_tax_amount) || 0), 0);
      const total_amount = Number((subtotal + vatTotal - discount - whtTotal).toFixed(2)) || 0;
      const sanitizedItems = validItems.map(i => {
        ...
        return {
          ...
          withholding_tax_rate: Number(i.withholding_tax_rate) || 0,
          withholding_tax_amount: Number(i.withholding_tax_amount) || 0
        };
      });
        ...
        const loadedItems = (fullData.items || []).map((item: any) => {
          ...
          return {
            ...
            withholding_tax_rate: (item.withholding_tax_rate !== undefined && item.withholding_tax_rate !== null)
              ...
              : (prod?.sales_withholding_tax_rate ? Number(prod.sales_withholding_tax_rate) : 0),
            withholding_tax_amount: (item.withholding_tax_amount !== undefined && item.withholding_tax_amount !== null)
              ? Number(item.withholding_tax_amount)
              : Number((total * (((item.withholding_tax_rate !== undefined && item.withholding_tax_rate !== null) ? Number(item.withholding_tax_rate) : (prod?.sales_withholding_tax_rate ? Number(prod.sales_withholding_tax_rate) : 0)) / 100)).toFixed(2))
          };
        });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="p-2 md:px-4 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-md z-[70] gap-2 flex-wrap" dir={dir}>
            ...
            <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
              ...
              <button 
                ...
                onClick={() => {
                  if (editingReturn) {
                    handleTriggerEtaUpload(editingReturn);
                  } else {
                    showNotification(
                      language === 'ar' 
                        ? 'يرجى حفظ المرتجع أولاً ثم الضغط على زر الرفع' 
                        : 'Please save the return first, then click upload', 
                      'warning'
                    );
                  }
                }}
                disabled={isSaving || isSubmittingEta || (!!editingReturn?.eta_uuid && editingReturn?.eta_status === 'Valid')}
                className={`px-3 py-1 rounded-xl font-bold transition-all flex items-center gap-1.5 justify-center active:scale-95 shadow-sm text-[11px] whitespace-nowrap font-sans ${
                  editingReturn?.eta_uuid 
                    ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100' 
                    : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-200'
                }`}
                title={editingReturn?.eta_uuid ? (language === 'ar' ? 'تحديث حالة المرتجع من الضرائب' : 'Sync ETA') : (language === 'ar' ? 'رفع المرتجع إلى منظومة الضرائب المصرية' : 'Upload to ETA')}
              >
                {isSubmittingEta ? (
                ...
                <span>
                  {editingReturn?.eta_uuid 
                    ...
                    : (language === 'ar' ? 'رفع المرتجع' : 'Upload to ETA')}
                </span>
              </button>
              ...
              <form id="return-form" onSubmit={handleSubmit} className="space-y-1.5">
                ...
                <section className="bg-white p-2 md:p-2.5 rounded-xl border border-zinc-200 shadow-sm grid grid-cols-1 lg:grid-cols-4 gap-2 lg:gap-2.5 items-stretch">
                  ...
                  <div className="lg:col-span-1 flex flex-col justify-center space-y-1 p-0.5 lg:border-s lg:border-zinc-150 lg:ps-2.5">
                    ...
                    <div className="bg-zinc-50 rounded-lg p-1.5 border border-zinc-100 space-y-0.5">
                      ...
                      {isSalesWhtEnabled && (() => {
                        const calculatedWht = items.reduce((sum, i) => {
                          const qty = Number(i.quantity) || 0;
                          const price = Number(i.unit_price) || 0;
                        ...
                        if (calculatedWht <= 0) return null;
                        return (
                        ...
                        <div className="flex flex-col items-end">
                          <span className="font-black text-xs tracking-tighter text-left">
                            {formatMoney(
                              ...
                              discount -
                              (isSalesWhtEnabled ? items.reduce((sum, i) => {
                                const qty = Number(i.quantity) || 0;
                                const price = Number(i.unit_price) || 0;
                                const rate = Number(i.withholding_tax_rate) || 0;
                                return sum + ((i.withholding_tax_amount !== undefined && i.withholding_tax_amount !== null && Number(i.withholding_tax_amount) > 0) ? Number(i.withholding_tax_amount) : (qty * price * (rate / 100)));
                  ...
                  <div className="overflow-x-auto rounded-xl border border-zinc-200 overflow-hidden">
                    <table className="w-full text-sm text-right border-collapse table-fixed min-w-[1300px]">
                      <thead>
                        <tr className="bg-zinc-100 border-b border-zinc-200 text-zinc-700 text-xs font-bold">
                          ...
                          <th className="p-1 border-r border-zinc-200 text-center w-24">{language === 'ar' ? 'سعر الوحدة' : 'Unit Price'}</th>
                              ...
                              {isSalesWhtEnabled && (
                                <>
                                  <th className="p-1 border-r border-zinc-200 text-center w-14">{language === 'ar' ? 'ض.خ.إ %' : 'WHT %'}</th>
                                  <th className="p-1 border-r border-zinc-200 text-center w-20">{language === 'ar' ? 'مبلغ ض.خ.إ' : 'WHT Amount'}</th>
                                </>
                              )}
                      ...
                      <tbody className="divide-y divide-zinc-100">
                              ...
                              {(() => {
                                ...
                                const vatAmount = isVat
                                  ? ((item.vat_amount !== undefined && item.vat_amount !== null && Number(item.vat_amount) > 0)
                                      ...
                                      : (base * (vatRate / 100)))
                                  : 0;
                                const whtRate = isSalesWhtEnabled ? (Number(item.withholding_tax_rate) || 0) : 0;
                                const whtAmount = isSalesWhtEnabled
                        ...
                        {items.length === 0 && (
                          <tr>
                            <td colSpan={10 + ((company?.settings?.vat_enabled !== false && company?.vat_enabled !== false) ? 2 : 0) + (isSalesWhtEnabled ? 2 : 0)} className="px-3 py-6 text-center text-zinc-400 italic text-xs font-sans">
                ...
                <div className="flex gap-4 pt-6 mt-6 border-t border-zinc-100">
                  ...
                  {editingReturn && (
                    <button 
                      type="button"
                      onClick={() => handleTriggerEtaUpload(editingReturn)}
                      disabled={isSaving || isSubmittingEta || (!!editingReturn?.eta_uuid && editingReturn?.eta_status === 'Valid')}
                      className={`py-3 px-5 rounded-xl font-bold transition-all shadow-md active:scale-95 flex items-center justify-center gap-2 text-sm ${
                        editingReturn?.eta_uuid 
                          ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100' 
                          : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-200'
                      }`}
                    >
                      <UploadCloud size={16} className={isSubmittingEta ? 'animate-bounce' : ''} />
                      <span>
                        {editingReturn?.eta_uuid 
                          ? (language === 'ar' ? 'تحديث بالضرائب' : 'Sync ETA') 
                          : (language === 'ar' ? 'رفع المرتجع إلكترونياً' : 'Submit Return to ETA')}
                      </span>
                    </button>
                  )}
                  <button 
                  ...
                  <div className="border border-[#f4f4f5] rounded-2xl overflow-hidden">
                    <table className={`w-full ${dir === 'rtl' ? 'text-right' : 'text-left'} text-sm`}>
                      ...
                      <tfoot className="bg-slate-50/50 font-bold border-t border-slate-100">
                        ...
                        {(Number(viewReturn.tax || (viewReturn as any).tax_amount || 0) > 0 || (viewReturn.items || []).some((i: any) => Number(i.vat_amount || 0) > 0)) && (
                          <tr>
                            ...
                            <td className="px-6 py-3 text-zinc-750 text-base">+{formatMoney(viewReturn.tax || (viewReturn as any).tax_amount || viewReturn.items?.reduce((sum: number, i: any) => sum + (Number(i.vat_amount) || 0), 0))} {viewReturnCurrency}</td>
                          </tr>
                        )}
                        {isSalesWhtEnabled && (Number((viewReturn as any).withholding_tax_amount || 0) > 0 || (viewReturn.items || []).some((i: any) => Number(i.withholding_tax_amount || 0) > 0)) && (
                          <tr>
                            <td colSpan={4} className={`px-6 py-3 ${dir === 'rtl' ? 'text-left' : 'text-right'} text-amber-600 font-bold text-[10px] uppercase tracking-wider`}>{language === 'ar' ? 'ضريبة الخصم والإضافة (ض.خ.إ)' : 'Withholding Tax'}</td>
                            <td className="px-6 py-3 text-amber-600 text-base">-{formatMoney((viewReturn as any).withholding_tax_amount || viewReturn.items?.reduce((sum: number, i: any) => sum + (Number(i.withholding_tax_amount) || 0), 0))} {viewReturnCurrency}</td>
                          </tr>
                        )}
      ...
      {/* ETA Validation Popup Modal (Combined Tax Number & Items Check) */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium mt-0.5">
                    {language === 'ar' ? 'يرجى استيفاء المتطلبات التالية قبل رفع المرتجع' : 'Please resolve the following requirements before ETA upload'}
                  </p>
                </div>
              </div>
              <button 
                  ...
                  <div className="flex-1">
                    ...
                    {etaValidationModal.taxNumberError && (
                      <p className="text-xs text-rose-700/80 mt-1 font-medium leading-relaxed">
                        {language === 'ar'
                          ? 'تشترط منظومة الضرائب وجود رقم التسجيل الضريبي للعميل لرفع المرتجع الإلكتروني بنجاح.'
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="font-bold text-sm">
                        {etaValidationModal.itemsError 
                          ? etaValidationModal.itemsError 
                          : (language === 'ar' ? 'جميع أصناف المرتجع مسجلة ومعتمدة على منظومة الضرائب' : 'All return items are registered on ETA')}
                      </h4>
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
