                ...
                <tr key={supplier.id} className="hover:bg-zinc-50/50 transition-colors group">
                  ...
                  <td className="px-4 py-4">
                    <span className={`font-bold text-sm ${supplier.currentBalance > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                      {formatBalance(supplier.currentBalance)} ج.م
                    </span>
                  </td>
                </tr>
              ))}
            ...
            {!loading && filteredSuppliers.length > 0 && (
              <tfoot className="bg-zinc-900 text-white font-bold">
                <tr>
                  ...
                  <td className="px-4 py-4">{filteredSuppliers.reduce((sum, s) => sum + s.openingBalance, 0).toLocaleString()}</td>
                  <td className="px-4 py-4">{filteredSuppliers.reduce((sum, s) => sum + s.totalInvoices, 0).toLocaleString()}</td>
                  <td className="px-4 py-4">{filteredSuppliers.reduce((sum, s) => sum + s.totalReturns, 0).toLocaleString()}</td>
                  <td className="px-4 py-4">{filteredSuppliers.reduce((sum, s) => sum + s.totalDiscounts, 0).toLocaleString()}</td>
                  <td className="px-4 py-4">{filteredSuppliers.reduce((sum, s) => sum + s.totalVouchers, 0).toLocaleString()}</td>
                  <td className="px-4 py-4">{filteredSuppliers.reduce((sum, s) => sum + (s.journalCredit - s.journalDebit), 0).toLocaleString()}</td>
            ...
            <div key={supplier.id} className="p-4 space-y-3 active:bg-zinc-50 transition-colors">
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <div className="flex justify-between border-b border-zinc-50 pb-1">
                  ...
                  <span className="font-medium">{supplier.openingBalance.toLocaleString()}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-50 pb-1">
                  <span className="text-zinc-400">مشتريات (+):</span>
                  <span className="text-emerald-600 font-medium">{supplier.totalInvoices.toLocaleString()}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-50 pb-1">
                  <span className="text-zinc-400">مرتجع (-):</span>
                  <span className="text-rose-600 font-medium">{supplier.totalReturns.toLocaleString()}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-50 pb-1">
                  <span className="text-zinc-400">سداد (-):</span>
                  <span className="text-blue-600 font-medium">{supplier.totalVouchers.toLocaleString()}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-50 pb-1">
                  <span className="text-zinc-400">قيود (+/-):</span>
                  <span className="text-zinc-600 font-medium">{(supplier.journalCredit - supplier.journalDebit).toLocaleString()}</span>
                </div>
              </div>
