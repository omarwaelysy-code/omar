...
import { 
  ...
  Lock, LayoutGrid, List, FileCheck
} from 'lucide-react';
...
export const SalesOrders: React.FC = () => {
  ...
  const handleItemChange = (index: number, field: keyof SalesOrderItem, value: any) => {
    ...
    if (field === 'product_id') {
      ...
      item.unit_price = prod?.sale_price || 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingOrder) {
        ...
        await dbService.logActivity(user.id, user.username, user.company_id, ot('edit_order'), `${ot('order_updated')}: ${orderNumber}`, 'sales_orders', editingOrder.id);
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-400 tracking-tighter uppercase mb-2">{ot('form_customer')}</label>
                    <select
