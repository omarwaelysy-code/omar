...
import { Search, Trash2, X, Layers, User, Calendar, RotateCcw, ChevronDown, Check, AlertCircle, Info, ArrowLeftRight } from 'lucide-react';
...
export const CustomerSettlements: React.FC = () => {
  ...
  const getInvoiceSettlements = (inv: any) => {
    ...
    allJournalEntries.forEach(je => {
      ...
      je.items?.forEach((item: any, idx: number) => {
        if (item.customer_id === inv.customer_id && Number(item.credit) > 0) {
  ...
  useEffect(() => {
    ...
    // 2. Sales Returns
    allReturns.forEach(r => {
      if (r.customer_id === selectedCustomerId) {
  ...
  }, [selectedCustomerId, allInvoices, allReceipts, allReturns, allJournalEntries]);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500">إجراء تسويات حرّة ومباشرة للعميل لربط الحركات المدينة بالدائنة بالتزامن الكامل.</p>
        </div>
