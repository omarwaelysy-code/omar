...
interface ReceivedChequePostponeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export const ReceivedChequePostponeModal: React.FC<ReceivedChequePostponeModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  cheque
  ...
  const handleConfirmPostpone = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await receivedChequeService.postponeCheque(cheque.id, newDueDate, reason);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {isAr ? 'تأجيل استحقاق الشيك الوارد' : 'Postpone Received Cheque Due Date'}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {isAr ? 'تمديد تاريخ التحصيل بناء على طلب العميل' : 'Extend collection date upon customer/drawer request'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleConfirmPostpone} className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              rows={2}
              required
              placeholder={isAr ? "مثال: طلب العميل مهلة إضافية لتغطية الرصيد..." : "E.g. Customer requested additional time to fund account..."}
              value={reason}
