...
import { 
  ...
  TrendingUp, Calendar, Plus, Eye, Check, RefreshCw 
} from 'lucide-react';
...
export const ChequesDashboardTab: React.FC<ChequesDashboardTabProps> = ({
  ...
  const kpis = [
    {
      title: isAr ? 'إجمالي الشيكات' : 'Total Cheques',
    ...
    {
      title: isAr ? 'شيكات متأخرة الصرف' : 'Overdue Cheques',
    ...
    {
      title: isAr ? 'الالتزامات المستقبلية' : 'Future Obligations',
      ...
      badge: isAr ? 'قائم برسم الدفع' : 'Outstanding'
    },
    {
      title: isAr ? 'شيكات تم صرفها' : 'Cleared Cheques',
      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2.5">
            <span>{isAr ? 'لوحة تحكم الشيكات الصادرة' : 'Issued Cheques Dashboard'}</span>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            {isAr 
              ...
              : 'Monitor cash liquidity, banking liabilities, and due dates of cheques issued to suppliers'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto">
          ...
          <button
            onClick={onCreateNew}
            className="flex-1 md:flex-none px-5 py-2.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 transition-all cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            <span>{isAr ? 'تحرير شيك صادر جديد' : 'Issue New Cheque'}</span>
          </button>
        </div>
      </div>
              ...
              <div className="mt-3 flex items-baseline justify-between">
                <div>
                  ...
                  <span className="text-[11px] text-slate-400 font-medium">
                    {currencyLabel} ({kpi.count} {isAr ? 'شيك' : (kpi.count === 1 ? 'cheque' : 'cheques')})
                  </span>
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {isAr ? 'أقرب الشيكات استحقاقاً (واجبة المتابعة والصرف)' : 'Upcoming Due Cheques (Action & Clearance Required)'}
              </h3>
              <p className="text-xs text-slate-400">
                {isAr ? 'الشيكات القائمة المستحقة خلال الفترة الحالية والمتأخرة' : 'Outstanding cheques due in the current period and overdue'}
              </p>
            </div>
          </div>
          <span className="text-xs font-bold text-slate-500 font-mono">
            {upcomingCheques.length} {isAr ? 'شيك' : (upcomingCheques.length === 1 ? 'cheque' : 'cheques')}
          </span>
        </div>
        ...
        {upcomingCheques.length === 0 ? (
          <div className="py-12 text-center text-slate-400">
            ...
            <p className="text-sm font-medium">
              {isAr ? 'لا توجد شيكات مستحقة الصرف حالياً' : 'No cheques due for clearance currently'}
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className={`w-full ${isAr ? 'text-right' : 'text-left'} text-xs`}>
              <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 font-bold border-b border-slate-100 dark:border-slate-800">
                <tr>
                  <th className="px-6 py-3.5">{isAr ? 'رقم الشيك' : 'Cheque #'}</th>
                      ...
                      <td className="px-6 py-3.5">
                        {isOverdue ? (
                          ...
                          <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
                            {isAr ? 'قائم برسم الدفع' : 'Under Payment'}
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-3.5">
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/30 dark:hover:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 font-bold text-[11px] flex items-center gap-1 transition-colors cursor-pointer"
                          >
