      ...
      <div className="w-full max-w-[1240px] mx-auto px-6 sm:px-10 lg:px-12 relative">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center relative">
          ...
          >
            ...
            {/* Main Headline */}
            <h1 className="text-4xl sm:text-5xl lg:text-[56px] font-extrabold text-[#0B0F19] tracking-tight leading-[1.16]">
              {t('landing.hero.headline_1')}<br />
              {t('landing.hero.headline_2')}<br />
              {t('landing.hero.headline_3')} <span className="text-[#1B853A]">{t('landing.hero.headline_highlight')}</span>
            </h1>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-5">
              ...
              >
                <span>{t('landing.nav.log_in')}</span>
              ...
              >
                <span>{t('landing.nav.request_demo')}</span>
                {isRtl ? (
                  <ArrowLeft className="w-4 h-4 text-white stroke-[2.2] transition-transform duration-200 group-hover:-translate-x-0.5" />
                ) : (
